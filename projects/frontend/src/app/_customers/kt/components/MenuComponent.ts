      ...
      if (menuItems && menuItems.length > 0) {
        for (let i = 0; i < menuItems.length; i++) {
          ...
          if (menuObj && menuObj.getItemSubType(item) === 'dropdown') {
            ...
            const isSelect = ((e.target as HTMLElement)?.className?.length && (e.target as HTMLElement)?.className?.includes('ng-option')) 
              || ((e.target as HTMLElement)?.className?.length && (e.target as HTMLElement)?.className?.includes('ng-value-icon'));
            if (sub && (sub === e.target || sub.contains(e.target as HTMLElement) || isSelect)) {
              continue
            }
            menuObj.hide(item)
          }
